...
import org.apache.ignite.ml.clustering.dbscan.DBSCANVector;
 ...
 */
public class LabeledVectorSet2<L, Row extends DBSCANVector> extends Dataset<Row> implements AutoCloseable {
    /**
     * Default constructor (required by Externalizable).
     */
    public LabeledVectorSet2() {
    }
     ...
     */
    public LabeledVectorSet2(int rowSize, int colSize, boolean isDistributed) {
     ...
     * @param colSize Amount of attributes. Should be > 0.
     */
    public LabeledVectorSet2(int rowSize, int colSize) {
     ...
     * @param isDistributed Use distributed data structures to keep data.
     */
    public LabeledVectorSet2(int rowSize, int colSize, String[] featureNames, boolean isDistributed) {
     ...
     */
    public LabeledVectorSet2(Row[] data) {
        super(data);
    }

    /** */
    private void initializeDataWithLabeledVectors() {
        data = (Row[])new DBSCANVector[rowSize];
        for (int i = 0; i < rowSize; i++)
            data[i] = (Row)new DBSCANVector(emptyVector(colSize, isDistributed), null);
    }
     ...
     */
    public LabeledVectorSet2(Row[] data, int colSize) {
        super(data, colSize);
    }
     ...
     */
    public LabeledVectorSet2(double[][] mtx, double[] lbs) {
     ...
     * @param isDistributed Use distributed data structures to keep data.
     */
    public LabeledVectorSet2(double[][] mtx, double[] lbs, String[] featureNames, boolean isDistributed) {
        super();
        assert mtx != null;
        assert lbs != null;
        ...
        for (int i = 0; i < rowSize; i++) {
            ...
            data[i] = (Row)new DBSCANVector(emptyVector(colSize, isDistributed), lbs[i]);
    ...
    public double label(int idx) {
        DBSCANVector labeledVector = data[idx];
    ...
    public void setLabel(int idx, double lb) {
        DBSCANVector<Vector, Double> labeledVector = data[idx];
    ...
    public LabeledVectorSet2 copy() {
        LabeledVectorSet2 res = new LabeledVectorSet2(data, colSize);
